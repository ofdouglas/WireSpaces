...
**Revision:** 4 — supersedes revision 3 and the earlier per-interaction-Origin drafts  
**Date:** 2026-08-23  
**Scope:** Core participant identity, Wire initiation semantics, canonical source/destination addressing, the revised 48-bit canonical base descriptor, Endpoint address allocation, internal/device-private Wires, binding-mode migration, zero-configuration bring-up semantics, constrained Classical CAN 11-bit direction, and repository migration  
**Explicitly out of scope:** Mandatory remote-source authorization, mandatory permitted-source tables, mandatory static deployment manifests, controller election/failover protocols, cryptographic authentication, multicast/group addressing, and final resolution of multi-Wire multiplexing in the compact CAN11 profile
...
For Classical CAN 11-bit, the current preferred direction intentionally accepts a compact-profile limitation instead of adding a Participant alias table: only a small low range of deployment-global Participant IDs is directly representable. That limitation is acceptable because WireSpaces also intends to support 29-bit CAN and richer Links. The compact CAN11 profile is therefore a deliberately constrained compatibility profile, not the canonical capacity floor for the whole system.
...
For the initial CAN11 compact profile, this proposal nevertheless chooses a direct low-global-ID restriction because the project is willing to accept the resulting deployment-wide CAN11 limit and provide CAN29/richer profiles as the escape path. That trade must be documented as a limitation, not described as capability-equivalent to a carrier-local alias scheme.
...
## 4.3 Why `8/8/8/{2|14}` instead of `10/10/10/10`
...
Revision 3 gave roughly 1,000 values to each deployment-local routing field while reducing EndpointId to 1,024 values per Namespace.

This revision instead gives:
...
A public registry is a one-way door: assignments may be reserved, deprecated, partitioned into ranges, or allocated to third parties and should not be expected to be reclaimed. Spare Endpoint space is therefore more valuable than excess machine-local routing capacity.
...
# 12. Classical CAN 11-bit compact profile direction

Classical CAN 11-bit remains the project's most constrained important profile.

It does not carry the full 48-bit canonical descriptor literally in its arbitration identifier.

## 12.1 Intentional deployment-wide compact-ID restriction

The initial compact CAN11 direction intentionally represents **low global Participant IDs directly**, without a Participant alias table.
...
A future profile may add local projections if real deployments show the low-global-ID budget is too restrictive.

## 12.2 Conceptual 11-bit identifier budget

Current preferred budget:
...
The exact physical bit order remains a `LINK` decision, except that QoS must retain the intended arbitration significance and commissioning must retain the intended low priority.

## 12.3 Direct PID ranges
...
Therefore the set of Participants eligible to appear in this compact CAN11 profile is drawn from the **deployment-global low PID range**.

Recommended allocation policy:

> Reserve low Participant IDs preferentially for Endpoint Domains that require compact CAN11 visibility. Do not casually consume them on Ethernet-only/internal-only Participants.

This is deployment allocation guidance, not a second identity namespace.

## 12.4 Representability rule

An ordinary addressed PDU is representable in the compact CAN11 form only if:

1. both Participants are in PID range `0..30`; and
2. at least one endpoint is in PID range `0..3`.

Thus:

- up to four low-ID controller/peer Participants may occupy the compact field;
- up to 31 global Participants are eligible for CAN11 compact visibility;
- traffic between two Participants both outside `0..3` is not representable in this profile;
- a deployment requiring such communication uses CAN29/richer profile/another Link or revisits the profile design.

A physical CAN segment therefore does **not** necessarily need every device to be compact-range, but every encoded pair needs one compact endpoint.

## 12.5 Direction is Link-local compression information
...
Direction has no canonical request/reply meaning.

## 12.6 Deterministic encoding when both Participants are compact

If both Participants are in `0..3`, use one deterministic representation.
...
This eliminates an arbitrary encoding choice without configuration.

Freeze this only with golden vectors in `LINK`/`CONFORM`.

## 12.7 Broadcast restriction

With `GeneralParticipantId == 31` representing broadcast, the compact profile's natural broadcast form is:

```text
CompactParticipantId = source PID 0..3
GeneralParticipantId = 31
Direction             = CompactToGeneral
```

Therefore this initial compact CAN11 form allows **only PID 0..3 Participants to originate true Wire-wide CAN11 broadcast**.

A general-range leaf may still send addressed status/publication to a compact Participant, and other devices may physically observe according to Link/profile policy, but that is not canonical Wire-wide broadcast.

This is an explicit profile limitation and must be judged against real CAN use cases before profile freeze.

## 12.8 Coexistence profile

When the CAN bus must coexist with arbitrary/non-WS 11-bit CAN traffic:

```text
ProfileBit = WS discriminator
```

The deployment/user takes responsibility for:

- choosing compatible low Participant IDs;
- avoiding conflicting CAN-ID use with non-WS traffic;
- establishing Participant IDs by some mechanism other than this profile's in-band commissioning channel.

This does not imply manual assignment. IDs may come from generated firmware, manufacturing data, board straps, another management Link, etc.

## 12.9 WS-exclusive profile

When WireSpaces owns the 11-bit CAN identifier space:

```text
ProfileBit = ordinary-data vs commissioning/control discriminator
```

The recovered bit may carry the in-band pre-addressing/commissioning channel.

Carry forward these requirements:

- unconfigured devices emit no ordinary WS Service traffic;
- selection/commissioning is below Wire semantics;
- commissioning traffic is structurally distinguishable from ordinary traffic;
- commissioning traffic should arbitrate below all ordinary traffic, including ordinary Background traffic where practical;
- commissioning establishes canonical Participant IDs rather than permanent CAN aliases.

The previously explored anonymous-response/prefix-search algorithm may be adapted but is not frozen by this change.

No anonymous canonical ParticipantId is required for commissioning control.

## 12.10 Profile meaning is a Link invariant

The final bit's meaning is defined by the selected Link profile, not negotiated independently per participant:
...
All WS participants on one bus use the same interpretation.

## 12.11 Gating issue: Wire identity on CAN11

The proposed 11-bit participant budget contains no old 3-bit `WireAlias` field.

This is now the principal unresolved blocker for declaring the compact CAN11 profile complete.

WireSpaces must not silently collapse “Wire” into “physical CAN bus” merely because the arbitration ID is full.

Candidate directions to evaluate in the CAN encoding/profile review:

1. **One native/local Wire in the arbitration ID, additional Wire identity in PDUA/data framing.**
2. **Single-Wire-per-compact-profile instance**, with a clearly stated loss of same-bus multi-Wire capability for that profile only.
3. **A richer CAN11 representation** that trades another property/capacity for Wire selection.
4. **Use CAN29 for deployments requiring rich same-bus Wire multiplexing.**

The core semantic change and 48-bit canonical descriptor do not depend on resolving this item, but `LINK` must not call CAN11 compact finalized until it is resolved.

## 12.12 Optimized N=1 CAN form must be revalidated

Current CAN optimization relies on compact Endpoint encoding and an identifier that already carries routing information.

The new canonical descriptor does not require CAN to place all 14 EndpointId bits literally into every short form.

Nevertheless, the CAN profile review must explicitly measure:

- N=1 payload capacity;
- compact Endpoint range;
- where Wire identity is carried;
- CRC/aggregation consequences;
- whether an N=1-only tiny node remains a valid first-class profile.
...
- compact PID `0..3`;
- general PID `0..30` or broadcast `31`;
- at least one addressed endpoint compact-range;
- deterministic both-compact representation;
- valid ProfileBit interpretation;
- valid Wire representation once §12.11 is resolved.
...
The planned RS-485 negative-control remap and old redundant-gateway remap were not completed. Record this as an evidence limitation.
...
Add the compact-profile direction from §12 but clearly label it **not complete until Wire identity is resolved**.

Preserve/revalidate:

- four QoS classes;
- arbitration ordering;
- low-priority commissioning requirement;
- PDUA/CRC/aggregation behavior;
- optimized N=1 capability;
- committed/guest/coexistence behavior where still applicable.
...
Do not add a Participant alias table in the initial compact CAN11 profile.

Explicitly document the deployment-wide low-PID restriction and direct-pair representability rule.
...
For CAN11 coexistence, no in-band PID commissioning is supplied by that profile.

For CAN11 exclusive, in-band commissioning may assign canonical PIDs.
...
Add CAN11 vectors only after §12.11 Wire representation is resolved, including:

- compact->general;
- general->compact;
- both-compact deterministic encoding;
- unrepresentable general->general;
- broadcast restriction;
- coexistence/exclusive ProfileBit semantics;
...
Rewrite old NodeId/broadcast-direction assumptions around the new profile distinction.

For CAN11 exclusive:

- commissioning/control uses profile-local control space;
- no canonical anonymous PID is required;
- assigned result is canonical ParticipantId.

For coexistence:

- commissioning is external to this profile.

Retain the previous priority requirement: commissioning should lose arbitration to all ordinary traffic where the final bit ordering permits it.
...
## 20.2 CAN11 Wire representation — gating for compact `LINK` profile

Resolve §12.11 before calling CAN11 compact implementation-ready.

This does not block host/sim/core semantic implementation.

## 20.3 CAN11 broadcast-source limitation — explicit profile review

Decide whether “only PID 0..3 may originate true Wire-wide broadcast” is acceptable for the compact profile.

If not, the profile must be changed; do not hide the limitation behind observer semantics.

## 20.4 Header-extension forwarding semantics

Resolve before the first extension with behavior-critical meaning is standardized.

## 20.5 Optional source admission

Keep open. Do not block core implementation.

## 20.6 Degraded/reachability reporting
...
## Step 6 — resolve CAN11 Wire representation
...
## Step 7 — implement CAN11 compact and CAN29 richer profiles

CAN29 should be treated as the normal escape path for deployments that exceed the intentional CAN11 compact limits.
...
- [ ] Is at-least-one-PID-0..3 representability explicit?
- [ ] Is only-PID-0..3 broadcast-source limitation explicit?
- [ ] Is Direction clearly profile-local?
- [ ] Is both-compact encoding deterministic?
- [ ] Are coexistence and exclusive modes separate profiles?
- [ ] Is the responsibility boundary for ID assignment clear?
...
22. The initial CAN11 compact direction deliberately uses low global PIDs and accepts a deployment-wide ~31-participant compact-visibility limit.
23. CAN11 additionally requires at least one endpoint in PID 0..3 for addressed traffic; true compact-profile broadcast is initially limited to PID 0..3 sources.
24. CAN11 coexistence spends the final bit on WS discrimination and requires IDs to be established elsewhere.
25. CAN11 WS-exclusive operation may spend that bit on commissioning/control.
26. CAN11 multi-Wire representation remains a gating `LINK` design item and is not silently collapsed.
27. CAN29 and richer Links are the intended escape path when CAN11 compact limits are unsuitable.
28. No backward-compatibility NodeId/Origin shim is required because interoperability is not frozen and the prototype has not implemented the canonical PDU stack.
