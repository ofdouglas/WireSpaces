...
def encode_hdlc_frame(frame: bytes) -> bytes:
    """Add flags and HDLC byte stuffing to one frame body."""
...
class HdlcStreamDecoder:
    """Incrementally decode byte-stuffed HDLC frames from arbitrary chunks."""
    ...
    def feed(self, data: bytes) -> Iterator[bytes]:
        """Consume serial bytes and yield complete unescaped frame bodies."""
        for byte in data:
            if byte == HDLC_FLAG:
                if self._in_frame and self._frame and not self._escaped:
                    yield bytes(self._frame)
                self._frame.clear()
                self._in_frame = True
                self._escaped = False
                continue
            ...
            if len(self._frame) > self._maximum_frame_size:
                LOGGER.warning(
